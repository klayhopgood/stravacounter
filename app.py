...
def save_tokens_to_db(athlete_id, access_token, refresh_token, expires_at, owner_id):
    try:
        ...
        """, (athlete_id, access_token, refresh_token, expires_at, athlete_id))
        connection.commit()
...
def get_tokens_from_db(owner_id):
    try:
        ...
        cursor.execute("SELECT access_token, refresh_token, expires_at FROM strava_tokens WHERE owner_id = %s",
                       (owner_id,))
...
def handle_activity_create(activity_id, owner_id):
    ...
    avg_pace, avg_pace_week = calculate_pace_stats(activities)
    beers_burnt, pizza_slices_burnt = calculate_food_burnt(activities)
    ...
    new_description = (
        ...
        f"Total kms run this year: {total_kms_run:.1f}\n"
        f"Average kms per week (last 4 weeks): {avg_kms_per_week:.1f}\n"
        f"Total elevation gain this year: {total_elevation}\n"
        f"Average elevation per week (last 4 weeks): {avg_elevation_per_week}\n"
        f"Average pace this year: {avg_pace}\n"
        f"Average pace per week (last 4 weeks): {avg_pace_week}\n"
...
def calculate_elevation_stats(activities):
    ...
    for activity in activities:
        if activity['type'] == 'Run':
            ...
            if today - datetime.timedelta(weeks=4) <= activity_date <= today:
                elevation_last_4_weeks += elevation_gain
...
def calculate_pace_stats(activities):
    ...
    for activity in activities:
        if activity['type'] == 'Run':
            ...
            if today - datetime.timedelta(weeks=4) <= activity_date <= today:
                total_seconds_week += moving_time
                total_distance_week += distance

    avg_pace = total_seconds / total_distance if total_distance else 0
    avg_pace_week = total_seconds_week / total_distance_week if total_distance_week else 0
    return avg_pace, avg_pace_week


def calculate_food_burnt(activities):
    ...
    return beers_burnt, pizza_slices_burnt
